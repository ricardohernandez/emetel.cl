...
<script type="text/javascript">
  $(function(){
    ...
    $("#menu_reporte").addClass('disabled_sub');
    $(".contenedor_app").html("<center><i id='processingIcon' class='fa-solid fa-circle-notch fa-spin fa-2x'></i></center>");
    $(".menu_lista li").removeClass('menuActivo');       
    $("#menu_reporte").addClass('menuActivo');  

    $.get("vistarkm", function( data ) {
      $(".contenedor_app").html(data);    
      $("#menu_reporte").removeClass('disabled_sub');
    });


    $(document).off('click', '#menu_reporte').on('click', '#menu_reporte',function(event) {
    ...
    <div class="col-12">
      <div class="scrollable-menu">
        <ul class="nav nav-tabs navbar-left nav-tabs-int menu_lista">
          <li id="menu_reporte" class="active"><a> <i class="fa fa-list-alt"></i> Reporte </a></li>   
        </ul>  
      </div> 
      </div> 
  </div>
